...
public class ReduceJoinDriver {
    public static void main(String[] args) throws IOException, InterruptedException, ClassNotFoundException {
        ...
        job.setOutputValueClass(DeliveryBean.class);
        //6. 指定job处理的原始数据路径
//        FileInputFormat.setInputPaths(job, new Path(args[0]));//指定读取数据的原始路径
        FileInputFormat.setInputPaths(job, new Path("/Users/liuwei/Documents/code/data_engineer/hadoop_client/resources/reducejoin"));//指定读取数据的原始路径
        //7. 指定job输出结果路径
//        FileOutputFormat.setOutputPath(job, new Path(args[1]));//指定输出结果的路径
        FileOutputFormat.setOutputPath(job, new Path("/Users/liuwei/Documents/code/data_engineer/hadoop_client/resources/reducejoin/output"));//指定输出结果的路径
        //8. 提交作业
